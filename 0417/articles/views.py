...
EMOTIONS = [
    {'label': '\U0001F60A', 'value': 1},
    {'label': '\U0001F606', 'value': 2},
    {'label': '\U0001F621', 'value': 3},
    {'label': '\U0001F622', 'value': 4},
]
...
def recommend(request, article_pk):
    article = Article.objects.get(pk=article_pk)
    if article.recommend.filter(pk=request.user.pk).exists():
        article.recommend.remove(request.user)
    else:
        article.recommend.add(request.user)
